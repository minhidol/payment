const statusRevenueService = require('../services/status_revenue.service');
const constants = require('../../constants/constants');
const {rsError, rsSuccess} = require('../helpers/response');
var moment = require('moment');

const handleCreateStatusRevenue = async(req, res) => {
    try {
        const data = {...req.body};
        data.create_by = req.jwtDecode.username;
        data.create_date = moment(new Date()).format("DD/MM/YYYY HH:mm:ss")
        await statusRevenueService.createStatusRevenue(data);
